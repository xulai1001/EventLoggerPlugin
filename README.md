...
Its manifest declares optional linkage to `LegendScenarioAnalyzer` and `RamenScenarioAnalyzer`. EventLogger tracks Legend (scenario ID `10`) and Ramen (scenario ID `14`) sessions. Ramen display integration captures immutable data for `(single_mode_chara_id, turn)` and updates the producer-owned `EventLogger` Extra section for that display ID. Legend display integration is disabled. Missing analyzers do not prevent EventLogger from loading or using its own workspace; the Ramen bridge is guarded by `IPluginContext.IsPluginAvailable`.
...
The plugin references the latest stable `UmamusumeResponseAnalyzer` NuGet package and pins linked plugin sources with Git submodules. From the repository root after cloning:
